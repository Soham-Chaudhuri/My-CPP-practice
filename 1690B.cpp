...
int main()
{
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);int t;cin>>t;while(t--)
    {
      ...
      cin>>n;
      vlli a;
      vlli b;lli sa=0;lli sb=0;lli maxa=-1;
      loop(i,0,n-1)
      {
          lli x;
          cin>>x;
          a.pb(x);sa+=x;maxa=max(maxa,x);
      }
      lli t=max_element(a.begin(),a.end())-a.begin();
      loop(i,0,n-1)
      {
          lli x;
          cin>>x;
      ...
      bool p=true;;lli diff=abs(maxa-b[t]);
      loop(i,0,n-1)
          {   
      if(b[i]>a[i])
      {
