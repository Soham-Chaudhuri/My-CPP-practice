...
void f() {
    ...
    auto cmp = [&](pair<lli,lli> &a,pair<lli,lli> &b){
        lli suma = a.first+a.second;
        lli sumb = b.first+b.second;
        lli suba = max(a.first,a.second)-min(a.first,a.second);
        lli subb = max(b.first,b.second)-min(b.first,b.second);
        if(suma == sumb){
            return suba > subb;
        }
