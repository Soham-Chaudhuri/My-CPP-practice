...
int main()
{
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
    {
      ...
      while(i<=(l-4))
      {
          lli a=min(abs(s[i]-'A'),(26-abs(s[i]-'A')));//cout<<s[i]<<" "<<a<<endl;
          lli c=min(abs(s[i+1]-'C'),(26-abs(s[i+1]-'C')));//cout<<s[i+1]<<" "<<c<<endl;
          lli t=min(abs(s[i+2]-'T'),(26-abs(s[i+2]-'T')));//cout<<s[i+2]<<" "<<t<<endl;
          lli g=min(abs(s[i+3]-'G'),(26-abs(s[i+3]-'G')));//cout<<s[i+3]<<" "<<g<<endl;
          maxs=min((a+c+t+g),maxs);
          i++;
