...
//lli y=b.to_ulong();
// pair<char, lli> findEntryWithLargestValue(
//     map<char, lli> sampleMap)
// {
//     pair<char, lli> entryWithMaxValue
//         = make_pair(0, 0);
//     map<char, lli>::iterator currentEntry;
...
void f(){
    ...
    loop(i,0,n){
        vlli mp(10,0);
        lli d=0;
        for(int j=i;j<min(100ll+i,n);j++){
            if(mp[s[j]-'0']==0) d++;
            mp[s[j]-'0']++;
            if(*max_element(all(mp))<=d)  ans++;
        }
    }
    cout<<ans<<endl;
}
