...
bool possible(lli target,lli a,lli b,lli c,lli d,lli e,lli f,lli g,lli h,lli j,lli i)
{
    lli bread=max(0ll,h*target-a);
    lli sausage=max(0ll,j*target-b);
    lli cheese=max(0ll,i*target-c);
    lli price=bread*d+sausage*e+cheese*f;
    return (price<=g);
}
int main()
{
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
    {
      ...
      {
          lli mid=(l+u)/2;
          if(possible(mid,nb,ns,nc,pb,ps,pc,r,cb,cs,cc))
          {
              ans=mid;
              l=mid+1;
          }
