...
void f(){
    ...
    cin>>n;
    vlli v(n),c;
    lli cnt1=0,cnt,cnt2=0;
    loop(i,0,n-1){
        cin>>v[i];
        cnt1+=countFactorsOfTwo(v[i]);
        cnt=countFactorsOfTwo(i+1);
        cnt2+=cnt;
        if(cnt!=0){
            c.pb(cnt);
        }
    }
    if(cnt1>=n) {cout<<0<<endl;return;}
    if(cnt1+cnt2<n) {cout<<-1<<endl;return;}
