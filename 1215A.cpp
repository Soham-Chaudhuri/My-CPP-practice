...
#define bloop(i,a,b) for(lli i=a;i>=b;i--)
#define pb push_back
#define pop pop_back
...
void pairsort(lli a[], lli b[], lli n){pair<lli, lli> pairt[n];for (int i = 0; i < n; i++){pairt[i].first = a[i];pairt[i].second = b[i];}sort(pairt, pairt + n);for (int i = 0; i < n; i++){a[i] = pairt[i].first;b[i] = pairt[i].second;}}
...
int main()
{
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);//int t;cin>>t;while(t--)
    {
      ...
      cin>>n;
      pairsort(k,a,2);
      lli cnt=a[0]*(k[0]-1)+a[1]*(k[1]-1);//cout<<cnt<<endl;cnt<=0||
      lli mins;
      lli maxs;
      if(n-cnt<0)
      {
