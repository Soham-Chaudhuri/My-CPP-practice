...
int main()
{
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);int t;cin>>t;while(t--)
    {
      ...
      lli n;
      cin>>n;
      vlli v;map<lli,lli> mp;set<lli> s;
      loop(i,0,n-1)
      {
          lli x;
          cin>>x;
          v.pb(x);mp[x]=0;s.insert(x);
      }
      sort(v.begin(),v.end());
      loop(i,0,n-1)
      {
        ...
        lli maxs=-99;lli key=0;lli cnt=0;
      loop(i,0,n-1)
      {
          if(mp[v[i]]>maxs)
          {
              maxs=max(maxs,mp[v[i]]);
              key=v[i];
          }
          if(mp[v[i]]==1)
          {
      ...
      lli ans;
      ans=max(min((lli)s.size(),maxs-1ll),min((lli)s.size()-1ll,maxs));
      if(n==1)
      {
          cout<<0<<endl;
      }
