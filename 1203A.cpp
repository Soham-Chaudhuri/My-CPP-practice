...
void solve(){    
    ...
    for(int i=0;i<n-1;i++){
        if(v[i]==1){
            if(v[i+1]!=2 && v[i+1]!=n){cout<<"NO"<<endl;return;}
        }
        else if(v[i]==n){
            if(v[i+1]!=1 && v[i+1]!=n-1){cout<<"NO"<<endl;return;}
        }
        else{
            if(abs(v[i]-v[i+1])!=1){cout<<"NO"<<endl;return;}
        }
    }
    cout<<"YES"<<endl;
}
...
int main(){
   ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);int t;cin>>t;while(t--){solve();
    }
    return 0;
}